...
def give_info(curr, time_c):
    img_1 = np.zeros([75, 350, 1], dtype=np.uint8)
    img_1.fill(255)
    index_max = current_visitor.index(max(current_visitor))
    cv2.putText(img_1, 'Max visitors : ' + str(current_visitor[index_max]), (10, 25), font, 0.5, (0, 0, 255), 1, cv2.LINE_AA)
...
def filtering(original_frame, background):
    ...
    kernel_Cl = np.ones((20, 20), np.uint8)
...
# 150 is a constant
areaTH = (height * width) / 150
line_up = int(1 * (height / 4))
...
while cap.isOpened():
        ...
        if area > areaTH:
            ...
            if cy in range(0, height):
                for i in persons:
                    ...
                    if abs(cx - i.x) <= int(w / 15) and abs(cy - i.y) <= int(h / 15):
                        new = False
                        i.x, i.y = cx, cy
                        if i.state:
                            continue
    ...
    str_up = 'Left: ' + str(count_up)
    str_down = 'Entered: ' + str(count_down)
    ...
    cv2.putText(frame, str_up, (10, line_up-10), font, 0.5, (0, 0, 255), 1, cv2.LINE_AA)
    cv2.putText(frame, str_down, (10, line_down-10), font, 0.5, (255, 0, 0), 1, cv2.LINE_AA)
